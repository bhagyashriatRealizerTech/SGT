...
public final class ServerUtilities {
     ...
     */
    public static boolean register(final Context context, final String regId, final String empID) {
        //Log.i(TAG, "registering device (regId = " + regId + ")");

        ctx = context;
        long backoff = BACKOFF_MILLI_SECONDS + random.nextInt(1000);
        // Once GCM returns search_layout registration id, we need to register it in the
        // demo server. As the server might be down, we will retry it search_layout couple
        // times.
        for (int i = 1; i <= MAX_ATTEMPTS; i++) {
            ...
            try {
               /*displayMessage(context, context.getString(
                ...
                Log.d(Config.TAG, "Registered Successfully");
                Config.displayMessage(context, message);
                return true;
            } catch (Exception e) {
                // Here we are simplifying and retrying on any error; in search_layout real
    ...
    static void unregister(final Context context, final String regId) {
        ...
        try {
            ...
            // if the server tries to send search_layout message to the device, it will get
            // search_layout "NotRegistered" error message and should unregister the device.
